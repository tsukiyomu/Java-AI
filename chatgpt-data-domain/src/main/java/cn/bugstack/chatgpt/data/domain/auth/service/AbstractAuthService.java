...
// 生成32字节（256位）的安全随机密钥
private static final String DEFAULT_SECRET = generateSecureSecret(32); // 32字节

    // 直接使用原始字节避免冗余编解码
    private static final byte[] SECRET_BYTES = ensureKeyLength(DEFAULT_SECRET.getBytes(), 32);

    // 最终使用的算法实例
    private final Algorithm algorithm = Algorithm.HMAC256(SECRET_BYTES);


    private static String generateSecureSecret(int keySize) {
        try {
            KeyGenerator keyGen = KeyGenerator.getInstance("HmacSHA256");
            keyGen.init(keySize * 8, new SecureRandom()); // 参数为bit长度
    ...
    private static byte[] ensureKeyLength(byte[] key, int minLength) {
        if (key.length >= minLength) return key;

        // 使用HKDF扩展密钥（此处为示例，实际应使用完整实现）
        byte[] extendedKey = new byte[minLength];
        System.arraycopy(key, 0, extendedKey, 0, key.length);
        for (int i = key.length; i < minLength; i++) {
            extendedKey[i] = (byte) (extendedKey[i - key.length] ^ 0x55); // 简单示例逻辑
        }
        return extendedKey;
    }
    @Override
    ...
    protected String encode(String issuer, long ttlMillis, Map<String, Object> claims) {
        ...
        JwtBuilder builder = Jwts.builder()
                ...
                .setSubject(issuer)
                .signWith(SignatureAlgorithm.HS256, SECRET_BYTES);//这个地方是生成jwt使用的算法和秘钥
        if (ttlMillis >= 0) {
